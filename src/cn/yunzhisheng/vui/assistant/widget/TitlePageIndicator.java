...
//		}
		...
		canvas.drawText(getTitle(0), 0, getTitle(0).length(), bounds.get(0).left, bounds.get(0).bottom
		...
		canvas.drawText(
			...
			getTitle(1).length(),
			bounds.get(1).left,
			bounds.get(1).bottom + mTopPadding,
			mPaintText);
		...
		// // in/out
		// mPaintText.setAlpha(colorTextAlpha - (int) (colorTextAlpha *
		// selectedPercent));
		// }
		//
		// // Except if there's an intersection with the right view
		...
		// }
		// }
		// canvas.drawText(pageTitle, 0, pageTitle.length(), 200, bound.bottom +
		// mTopPadding, mPaintText);
		//
		// // If we are within the selected bounds draw the selected text
		// if (currentPage && currentSelected) {
		// mPaintText.setColor(mColorSelected);
		// mPaintText.setAlpha((int) ((mColorSelected >>> 24) *
		// selectedPercent));
		// canvas.drawText(pageTitle, 0, pageTitle.length(), 100, bound.bottom +
		// mTopPadding, mPaintText);
		// }
		// }
		// }
		...
		case Underline:
			...
			final float leftMinusPadding = underlineBounds.left - 5;
			final float rightPlusPadding = underlineBounds.right + 5;
			// final float rightPlusPadding = leftMinusPadding+150;
	...
	public boolean onTouchEvent(android.view.MotionEvent ev) {
		...
		case MotionEvent.ACTION_UP:
			if (!mIsDragging) {
				...
				if (eventX < calculateAllBounds(mPaintText).get(1).left
					&& eventX > (calculateAllBounds(mPaintText).get(1).left * 2 - calculateAllBounds(mPaintText).get(1).right)) {
					if (mCurrentPage > 0) {
				...
				} else if (eventX > calculateAllBounds(mPaintText).get(1).left) {
...
//	}
	...
	private ArrayList<Rect> calculateAllBounds(Paint paint) {
		...
		for (int i = 0; i < count; i++) {
			...
			// bounds.left = (int) (halfWidth - (w / 2f) + ((i - mCurrentPage -
			// mPageOffset) * width));
			...
			//
			bounds.left = (int) (width - 100 - ((1-i) * 75));
