...
public class RomCustomMessageReceiver extends BroadcastReceiver {
	...
	public void onReceive(Context context, Intent intent) {
		...
		if (action.equals(CATION_CUSTOM_UPDATE)) {// 添加蓝牙电话传输过来的电话到数据库
			...
			LogUtil.d(TAG, "CATION_CUSTOM_UPDATE flag:" + flag);
			switch (flag) {
			case CONTACT_SYNC:
...
//		LogUtil.d(TAG, "name: " + name + " number: " + number);
		ContentResolver resolver = mContext.getContentResolver();
		Uri uri = Uri
				.parse("content://cn.yunzhisheng.vui.provider.ContactProvider/contact");
...
//					LogUtil.d(TAG, " contact_id:" + contact_id);
			...
			if (contactdIds.size() > 0) {
				for (long id : contactdIds) {
					if (deletePhone(id, number)) {
						...
						LogUtil.d(TAG, CATION_CUSTOM_UPDATE + " deleted");
					}
				}
			}
		}
	...
	private void insertContactInfosInNewThread(final ArrayList<String> contacts) {
		...
		Log.d("mg", "语音的联系人添加"+contacts.toString());
		if (mThread == null) {
				...
				public void run() {
					if (contacts != null && contacts.size() > 1) {
						...
						for (int i = 0; i < contacts.size(); i++) {
							String[] info = contacts.get(i).split("_");
							LogUtil.d(TAG, "info : " + Arrays.asList(info));
							if (info != null && info.length == 2) {
								insertContact(info[0], info[1]);
							}
						}
						Intent intent=new Intent(RomCustomSetting.PHONEBOOK_OK);
						mContext.sendBroadcast(intent);
					}
					LogUtil.d(TAG, CATION_CUSTOM_UPDATE + " sync end");
					mThread = null;
				}
			});
		}
		if (!mThread.isAlive()) {
			LogUtil.d(TAG, CATION_CUSTOM_UPDATE + " start sync");
			mThread.start();
		}
	}
}
