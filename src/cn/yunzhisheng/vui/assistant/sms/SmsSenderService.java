...
import cn.yunzhisheng.vui.assistant.model.Telephony.Sms;
...
public class SmsSenderService extends Service {
	...
	public static final String ACTION_SERVICE_STATE_CHANGED = "cn.yunzhisheng.intent.action.SERVICE_STATE";
	public static final long NO_TOKEN = -1L;
	private ServiceHandler mServiceHandler;
	private Looper mServiceLooper;
	private boolean mSending;

	public static final String MESSAGE_SENT_ACTION = "cn.yunzhisheng.intent.action.MESSAGE_SENT";

	// Indicates next message can be picked up and sent out.
	public static final String EXTRA_MESSAGE_SENT_SEND_NEXT = "SendNextMsg";

	public static final String ACTION_SEND_MESSAGE = "cn.yunzhisheng.intent.action.SEND_MESSAGE";
	...
	public int onStartCommand(Intent intent, int flags, int startId) {
		...
		if (mResultCode != 0) {
			LogUtil.v(TAG, "onStart: #" + startId + " mResultCode: " + mResultCode + " = " + translateResultCode(mResultCode));
		}
	...
	private final class ServiceHandler extends Handler {
		...
		public void handleMessage(Message msg) {
			...
			LogUtil.v(TAG, "handleMessage serviceId: " + serviceId + " intent: " + intent);
			...
			if (intent != null) {
				...
				LogUtil.v(TAG, "handleMessage action: " + action + " error: " + error);
	...
	public synchronized void sendFirstQueuedMessage() {
		...
		Cursor c = SqliteWrapper.query(this, resolver, uri, SEND_PROJECTION, null, null, "date ASC"); // date
																										// ASC
																										// so
																										// we
																										// send
		...
		if (c != null) {
			try {
				if (c.moveToFirst()) {
					...
					LogUtil.v(TAG, "sendFirstQueuedMessage " + msgUri + ", address: " + address + ", threadId: " + threadId);

					try {
						sender.sendMessage(NO_TOKEN);
						mSending = true;
					} catch (SmsException e) {
						LogUtil.e(TAG, "sendFirstQueuedMessage: failed to send message " + msgUri + ", caught :" + e.getMessage());
						mSending = false;
		...
		if (success) {
			...
			mToastHandler.post(new Runnable() {
				public void run() {
					Toast.makeText(SmsSenderService.this, getString(R.string.smsMessageSent), Toast.LENGTH_SHORT).show();
				}
			});
		}
	}
	...
	private void handleSmsSent(Intent intent) {
		...
		LogUtil.v(TAG, "handleSmsSent uri: " + uri + " sendNextMsg: " + sendNextMsg + " mResultCode: " + mResultCode + " = " + translateResultCode(mResultCode));

		if (mResultCode == Activity.RESULT_OK) {

			LogUtil.v(TAG, "handleSmsSent move message to sent folder uri: " + uri);

			if (!Sms.moveMessageToFolder(this, uri, Sms.MESSAGE_TYPE_SENT)) {
				LogUtil.e(TAG, "handleSmsSent: failed to move message " + uri + " to sent folder");
			}
			if (sendNextMsg) {
				sendFirstQueuedMessage();
			}
			...
			LogUtil.v(TAG, "handleSmsSent: no service, queuing message w/ uri: " + uri);
	...
	private void messageFailedToSend(Uri uri) {
		...
		LogUtil.v(TAG, "messageFailedToSend msg failed uri: " + uri);
	...
	private void registerForServiceStateChanges() {
		...
		LogUtil.v(TAG, "registerForServiceStateChanges");
		context.registerReceiver(SmsSendReceiver.getInstance(), intentFilter);
	}

	private void unRegisterForServiceStateChanges() {

		LogUtil.v(TAG, "unRegisterForServiceStateChanges");
